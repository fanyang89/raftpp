...
// CRC32C lookup table generated for polynomial 0x1EDC6F41 (reflected)
// This is the Castagnoli polynomial used in iSCSI, ext4, etc.
const uint32_t CRC32C::kTable[256] = {
...
uint32_t CRC32C::Compute(std::span<const uint8_t> data) {
    return Compute(data.data(), data.size());
}

uint32_t CRC32C::Extend(uint32_t crc, const void* data, size_t len) {
    CRC32C c(crc);
    c.Update(data, len);
    return c.Finalize();
}
