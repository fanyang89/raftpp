...
void Raft::Campaign(std::string_view campaign_type) {
    ...
    for (const uint64_t id : progress_tracker_.conf().voters.IDs()) {
        ...
        m_builder.setTo(id);
        m_builder.setMsgType(capnp_util::as<msg::MessageType>(vote_msg));
...
void Raft::CommitApplyInternal(uint64_t applied, bool skip_check) {
    ...
    if (progress_tracker_.conf().auto_leave && old_applied <= pending_conf_index_ &&
        ...
        ent_builder.setEntryType(capnp_util::as<msg::EntryType>(EntryType::ENTRY_CONF_CHANGE_V2));
        ent_builder.setData(
...
void Raft::SendTimeoutNow(const uint64_t to) {
    ...
    auto m_builder = capnp_util::builder<msg::Message>(m);
    m_builder.setTo(to);
    m_builder.setMsgType(static_cast<msg::MessageType>(static_cast<int>(MessageType::MSG_TIMEOUT_NOW
    )));
    Send(m, messages_);
}
...
void Raft::SendRequestSnapshot() {
    auto m = capnp_util::make<msg::Message>();
    auto m_builder = capnp_util::builder<msg::Message>(m);
    m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_APPEND_RESPONSE));
...
void Raft::HandleHeartbeat(const Message& m) {
    ...
    to_send_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_HEARTBEAT_RESPONSE)
    );
...
void Raft::HandleSnapshot(const Message& m) {
    ...
    auto to_send = capnp_util::make<msg::Message>();
    auto to_send_builder = capnp_util::builder<msg::Message>(to_send);
    to_send_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_APPEND_RESPONSE));
    to_send_builder.setTo(m_reader.getFrom());
...
std::optional<Message> Raft::HandleReadyReadIndex(const Message& req, uint64_t index) {
    ...
    m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_READ_INDEX_RESP));
...
void Raft::SendHeartbeat(
    ...
    auto m_builder = capnp_util::builder<msg::Message>(m);
    m_builder.setTo(to);
    m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_HEARTBEAT));
    ...
    if (m_reader.getTerm() == 0) {
        ...
        if ((check_quorum_ || pre_vote_) &&
            ...
            to_send_builder.setMsgType(
                capnp_util::as<msg::MessageType>(MessageType::MSG_APPEND_RESPONSE)
            );
            Send(to_send, messages_);
            ...
            auto to_send_builder = capnp_util::builder<msg::Message>(to_send);
            to_send_builder.setTo(m_reader.getFrom());
            to_send_builder.setMsgType(static_cast<msg::MessageType>(
...
bool Raft::TickElection() {
    ...
    if (election_elapsed_ >= randomized_election_timeout_) {
        ...
        m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_HUP));
    ...
    if (heartbeat_elapsed_ >= heartbeat_timeout_) {
        ...
        auto m_builder = capnp_util::builder<msg::Message>(m);
        m_builder.setTo(kInvalidId);
        m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_BEAT));
        m_builder.setFrom(id_);
        std::ignore = Step(m);
    }
...
bool Raft::TickHeartbeat() {
    ...
    if (election_elapsed_ >= randomized_election_timeout_) {
        ...
        if (check_quorum_) {
            ...
            auto m_builder = capnp_util::builder<msg::Message>(m);
            m_builder.setTo(kInvalidId);
            m_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_CHECK_QUORUM));
            m_builder.setFrom(id_);
            has_ready = true;
            std::ignore = Step(m);
        }
...
Result<ConfState> Raft::ApplyConfChange(const ConfChangeV2& cc) {
    ...
    if (LeaveJoint(cc)) {
        ...
        for (const auto& c : changes_list) {
            ...
            single_builder.setChangeType(capnp_util::as<msg::ConfChangeType>(c.getChangeType()));
