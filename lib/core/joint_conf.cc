...
std::pair<uint64_t, bool> JointConfiguration::CommittedIndex(
...
VoteResult JointConfiguration::GetVoteResult(
...
fmt::context::iterator fmt::formatter<raftpp::JointConfiguration>::format(
    ...
    return fmt::format_to(
        ctx.out(), "[incoming: {}, outgoing: {}]", value.incoming(), value.outgoing()
    );
}
