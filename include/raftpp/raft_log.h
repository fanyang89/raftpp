...
class RaftLog {
  public:
    ...
    [[nodiscard]] std::optional<MaybeAppendResult> MaybeAppend(
        uint64_t idx, uint64_t term, uint64_t committed,
        const std::vector<Entry>& entries
    );
    [[nodiscard]] std::optional<std::vector<Entry>> NextEntriesSince(
        uint64_t since_idx, std::optional<uint64_t> max_size
    );
    [[nodiscard]] std::optional<std::vector<Entry>> NextEntries(
        std::optional<uint64_t> max_size
    );
    [[nodiscard]] std::pair<uint64_t, std::optional<uint64_t>>
    FindConflictByTerm(uint64_t index, uint64_t term) const;
    ...
    [[nodiscard]] uint64_t FindConflict(
        const std::vector<Entry>& entries
    ) const;
  ...
  private:
    [[nodiscard]] Result<void> MustCheckOutOfBounds(
    ...
    while (low < high) {
        if (const auto ents = Slice(low, high, page_size, ctx); !ents) {
            ...
            if (ents->empty()) {
                return RaftError(
                    StorageErrorOther{
                        fmt::format("got 0 entries in [{}, {})", low, high)
                    }
                );
            }
