            ...
            if (!entries.empty()) {
                ...
                for (const auto& e : existing_entries) {
                    auto entry = capnp_util::make<msg::Entry>([&](auto entry_builder) {
                        entry_builder.setEntryType(capnp_util::as<msg::EntryType>(e.getEntryType())
                        );
                ...
                for (size_t i = 0; i < all_entries.size(); ++i) {
                    ...
                    dst.setEntryType(capnp_util::as<msg::EntryType>(src_reader.getEntryType()));
...
void RaftCore::PrepareSendEntries(
    ...
    msg_builder.setMsgType(capnp_util::as<msg::MessageType>(MessageType::MSG_APPEND));
...
bool RaftCore::PrepareSendSnapshot(Message& m, Progress& pr, uint64_t to) {
    ...
    capnp_util::builder<msg::Message>(m).setMsgType(
        capnp_util::as<msg::MessageType>(MessageType::MSG_SNAPSHOT)
    );
