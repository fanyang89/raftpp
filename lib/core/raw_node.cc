...
void RawNode::TransferLeader(const uint64_t transferee) {
    ...
    builder.setMsgType(capnp_util::cast_enum<msg::MessageType>(MessageType::MSG_TRANSFER_LEADER));
...
void RawNode::ReadIndex(const std::string& ctx) {
    Message m = capnp_util::make<msg::Message>();
    auto builder = capnp_util::builder<msg::Message>(m);
    builder.setMsgType(capnp_util::cast_enum<msg::MessageType>(MessageType::MSG_READ_INDEX));
...
void RawNode::ReportUnreachable(uint64_t id) {
    Message m = capnp_util::make<msg::Message>();
    auto builder = capnp_util::builder<msg::Message>(m);
    builder.setMsgType(capnp_util::cast_enum<msg::MessageType>(MessageType::MSG_UNREACHABLE));
    builder.setFrom(id);
    std::ignore = raft_.Step(m);
}
...
void RawNode::ReportSnapshot(const uint64_t id, const SnapshotStatus status) {
    ...
    Message m = capnp_util::make<msg::Message>();
    auto builder = capnp_util::builder<msg::Message>(m);
    builder.setMsgType(capnp_util::cast_enum<msg::MessageType>(MessageType::MSG_SNAP_STATUS));
    builder.setFrom(id);
